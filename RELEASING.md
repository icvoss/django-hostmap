...
How to cut a release for django-hostmap.
...
2. **Bump** on the branch:
   ...
   - `src/hostmap/__init__.py` - update `__version__` (keep in sync)
...
6. **Watch the publish run** and confirm PyPI:
   ...
   curl -s https://pypi.org/pypi/django-hostmap/json | python -c \
...
- `src/hostmap/__init__.py` -> `__version__`
...
- [ ] Version bumped in `pyproject.toml` **and** `src/hostmap/__init__.py`,
      and they match.
